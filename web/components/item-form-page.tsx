...
function toFormState(
  ...
  return {
    ...
    targetPrice:
      ...
      "",
    comparisonQuantity:
      item?.comparisonQuantity?.toString() ??
      "1",
    lastPurchasePrice:
    ...
    updateMode:
      item?.updateMode ?? "Automatic",
    checkIntervalMinutes:
      item?.checkIntervalMinutes?.toString() ??
      "",
    trackingEnabled:
  ...
  return item.sources.map(
    (source) => ({
      ...
      kind:
        ...
        source.manualPrice !== null
          ? "manual"
          : "automatic",
      defaultQuantity:
        source.defaultQuantity.toString(),
      manualPrice:
        source.manualPrice?.toString() ??
        "",
      note: source.note ?? "",
    })
  );
...
export function ItemFormPage({
  ...
  const [sources, setSources] =
    useState<SourceDraft[]>(
      toSourceDrafts(initialItem)
    );
  ...
  async function save() {
    ...
    try {
      ...
      for (const source of sources) {
        ...
        if (source.id) {
          await apiFetch<void>(
            `/api/items/${itemId}/sources/${source.id}`,
            token.accessToken,
            {
              method: "PUT",
              body: JSON.stringify(
  ...
  function validate() {
    ...
    if (
      ...
      throw new Error(
        "Comparison quantity must be greater than zero."
      );
    }
    ...
    for (const source of sources) {
      ...
      if (
        ...
        throw new Error(
          `Source "${source.store}" needs a quantity greater than zero.`
        );
      }
      ...
      if (
        ...
        throw new Error(
          `Manual source "${source.store}" needs a manual price.`
        );
      }
    }
  }
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        ...
        <div className="mt-4">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Raw package price and quantity
            stay in the data model; the
            website compares normalized
            unit prices.
          </p>
        </div>
